...
var (
	...
	ErrMergeBaseMissing    = errors.New("three-way merge requires a previous backup as the merge base")
...
func Apply(opts Options) (*Result, error) {
	...
	case PolicyFail:
		...
		return result, ErrConflictRefused
	case PolicyThreeWay:
		merged, mergeErr := applyThreeWay(localEnv, incomingEnv, opts)
		if mergeErr != nil {
...
func applyThreeWay(localEnv, incomingEnv *envfile.EnvFile, opts Options) (*envfile.EnvFile, error) {
	if opts.ProjectID == "" {
	...
	latest, err := vStore.Latest(opts.ProjectID)
	if err != nil {
		return nil, fmt.Errorf("loading backup manifest: %w", err)
	}
	if latest == nil {
		return nil, ErrMergeBaseMissing
	}

	baseData, err := vStore.Restore(opts.ProjectID, latest.Sequence, opts.BackupKey)
	if err != nil {
		return nil, fmt.Errorf("loading merge base from backup %d: %w", latest.Sequence, err)
	}
